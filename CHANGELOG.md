...
- `src/near.rs`: `Hosting::{Tee,Proxied}`, `ModelInfo::is_private()`, redacting `ApiKey` + 4 tests.
...
- `src/near_wire.rs`: chat JSON `ChatRequest`/`ChatResponse` + `Usage` + catalog `ModelEntry`/`ModelsResponse` with `estimated_cents()` + 5 tests.
...
- `src/main.rs`: CLI-driven — brief + monthly via `route_with_monthly()`; live Jev `evaluate()` → `answers_from_wire()` → route → NEAR `complete()` on catalog `cheapest()` priced by `estimate_plan()`, offline fallback.
- Deps: `serde` + `serde_json` + `ureq` with `json` feature (wire shapes + sync HTTP + live callers + CLI + live Jev loop + live catalog done, attestation next).
